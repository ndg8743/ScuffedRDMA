...
\midrule
Model & gpt-oss-120b (116.8B parameters) \\
Quantization & MXFP4 \\
Model Size & 63GB \\
...
TCP (baseline) & \texttt{NCCL\_NET\_GDR\_LEVEL=0} & Disable GPUDirect, use TCP sockets \\
 & \texttt{NCCL\_IB\_DISABLE=1} & Disable InfiniBand/RoCE \\
\midrule
...
\begin{enumerate}[noitemsep]
    ...
    \item \textbf{NCCL optimizes locally}: NCCL uses shared memory for intra-node communication
\end{enumerate}
...
\begin{enumerate}[noitemsep]
    ...
    \item \textbf{Hardware RoCE recommended}: Mellanox ConnectX would provide 100$\times$ better RDMA performance
\end{enumerate}
