...
SAE feature vectors are a natural fit for Neumann's \texttt{SparseVector} type, which stores only non-zero indices and values. A typical SAE feature vector has 65,536 dimensions but only approximately 50 active features, which illustrates the storage advantage: the dense representation requires $65{,}536 \times 4 = 256$\,KB, while Neumann's \texttt{SparseVector} stores only the active index--value pairs, consuming roughly $50 \times (4 + 4) = 400$\,bytes---a 51$\times$ compression ratio. These sparse vectors are constructed from the list of active feature indices and their activation magnitudes, stored in Neumann under descriptive keys (e.g., \texttt{sae:layer\_12:eiffel\_tower}), and made available for RDMA access by both towers. A single feature vector at approximately 400~bytes fits well within a single RDMA read, far below the network MTU.
...
The \texttt{RDMATensorCacheMiddleware} class extends the ScuffedRDMA middleware with Neumann tensor cache access. On initialization, it establishes a transport connection and creates a \texttt{NeumannClient} pointing to the Neumann host at 192.168.2.111 on port 4791, along with a \texttt{PrecisionRouter} configured with the available device capabilities. The middleware exposes two primary data-path methods. The \texttt{get\_tensor} method fetches a tensor from Neumann by key: it determines the optimal wire format for the requesting device via the precision router, issues an RDMA read through the Neumann client, and decodes the raw bytes into a PyTorch tensor in the appropriate precision. The \texttt{put\_gradient} method performs the reverse operation, encoding a PyTorch gradient tensor from the source device's native format and writing it to Neumann via RDMA for FP32 master weight accumulation. A \texttt{get\_kv\_cache} convenience method provides batch access to centralized KV cache entries by constructing keys from the layer index and sequence positions and issuing a batched RDMA read to retrieve the requested entries from Neumann's store.

\subsection{vLLM Connector}

The connector follows vLLM's existing patterns for distributed KV cache management. vLLM supports pluggable connectors (MooncakeConnector~\cite{mooncake2024}, NixlConnector~\cite{nixl}) through its transfer engine interface. Our \texttt{NeumannKVConnector} implements the same interface. It is initialized with a vLLM configuration and a role of either ``prefill'' or ``decode,'' and internally creates an \texttt{RDMATensorCacheMiddleware} instance. The \texttt{send\_kv\_caches\_and\_hidden\_states} method iterates over all layers' KV caches after prefill, writing each to Neumann under a key constructed from the request ID and layer index. The complementary \texttt{recv\_kv\_caches\_and\_hidden\_states} method, used during decode, reads KV caches back from Neumann for each layer using the middleware's \texttt{get\_kv\_cache} method, returning the assembled caches to vLLM's execution engine.
