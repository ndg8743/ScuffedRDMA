...
Two 2025--2026 systems target overlapping territory.
...
10\,GbE. The \texttt{sandook}~\cite{sandook} storage project at MIT
attacks a complementary bottleneck in the same inference pipeline:
it aggregates NVMe SSDs into a unified block device with
read/write workload isolation and latency-model-driven scheduling,
supplying the sort of NSD-class substrate that a KV-cache
offloading tier can sit on. Where TransferEngine and sandook
provide the raw transport and storage layers, libscuffedrdma adds
the tensor-aware classification (WFA), bandwidth allocation (PMP),
and compression (scuffedQuant) layers above them --- features
neither targets directly.
