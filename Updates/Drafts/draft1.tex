...
% Additional colors for GPU Communication Stack diagrams
\definecolor{rdmaBlue}{HTML}{1565C0}
\definecolor{ucxGreen}{HTML}{2E7D32}
\definecolor{ncclOrange}{HTML}{E65100}
\definecolor{frameworkPurple}{HTML}{6A1B9A}
\definecolor{middlewareRed}{HTML}{C62828}
\definecolor{consumerTeal}{HTML}{00838F}
\definecolor{enterpriseGold}{HTML}{F57F17}
\definecolor{hotRed}{HTML}{D32F2F}
\definecolor{warmYellow}{HTML}{FBC02D}
\definecolor{coldBlue}{HTML}{1976D2}
\definecolor{sectioncolor}{HTML}{0D47A1}
\definecolor{hardwareGray}{HTML}{546E7A}
...
\title{
    ...
    \large Draft 1: Consolidated Technical Report\\
    \normalsize (Updates 2--12 Unified)
}
...
%=============================================================================
\section{The GPU Communication Stack}
\label{sec:gpu-comm-stack}
%=============================================================================

Modern distributed LLM systems depend on a deep, fragile stack of communication technologies. Each layer was designed independently, often for different workloads, and none were built with the runtime adaptivity that LLM inference and training demand. As models scale beyond single-GPU capacity, the communication overhead between GPUs---both within and across nodes---becomes the dominant bottleneck. This section maps the full stack, identifies where it breaks for both consumer and enterprise users, and shows where adaptive tensor-aware middleware fits.

\subsection{Full Communication Stack Architecture}

\begin{figure}[htbp]
\centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
    layer/.style={draw, thick, rounded corners=4pt, minimum width=12cm, minimum height=1.5cm, font=\small, text=white, align=center},
    arrowstyle/.style={-{Stealth[length=3mm]}, thick},
    brace/.style={decorate, decoration={brace, amplitude=5pt, mirror}},
]

\node[layer, fill=hardwareGray] (hw) at (1,0) {
    \textbf{Layer 0: Physical Hardware}\\
    NVLink \textbar\ PCIe \textbar\ InfiniBand \textbar\ RoCE \textbar\ iWARP \textbar\ Ethernet
};

\node[layer, fill=rdmaBlue, above=0.35cm of hw] (rdma) {
    \textbf{Layer 1: RDMA Verbs \& Kernel Drivers}\\
    libibverbs / rdma-core \textbar\ OFED / MOFED \textbar\ SoftRoCE
};

\node[layer, fill=ucxGreen, above=0.35cm of rdma] (ucx) {
    \textbf{Layer 2: Unified Communication X (UCX)}\\
    Transport selection (IB, RoCE, TCP, shared\_mem) \textbar\ UCX-Py
};

\node[layer, fill=ncclOrange, above=0.35cm of ucx] (nccl) {
    \textbf{Layer 3: Collective Communication Libraries}\\
    NCCL (NVIDIA) \textbar\ RCCL (AMD) \textbar\ Gloo \textbar\ OpenMPI
};

\node[layer, fill=frameworkPurple, above=0.35cm of nccl] (fw) {
    \textbf{Layer 4: Distributed ML Frameworks}\\
    vLLM \textbar\ DeepSpeed \textbar\ Megatron-LM \textbar\ Ray \textbar\ mistral.rs
};

\node[layer, fill=middlewareRed, above=0.35cm of fw, minimum height=1.7cm, densely dashed] (mw) {
    \textbf{Proposed: Adaptive Tensor-Aware Middleware (Thesis)}\\
    WFA hot/warm/cold classification \textbar\ Runtime telemetry\\
    Dedicated low-latency QPs for KV cache \textbar\ Bulk channels
};

\node[layer, fill=sectioncolor!70, above=0.35cm of mw] (app) {
    \textbf{Layer 5: LLM Application}\\
    Training (gradient sync) \textbar\ Inference (KV cache, attention)
};

\draw[arrowstyle, color=hotRed] ([xshift=0.4cm]app.east |- app.south) -- ([xshift=0.4cm]hw.east |- hw.north)
    node[midway, right=3pt, font=\scriptsize\bfseries, text=hotRed, align=left] {
        Data Flow:\\
        tensors,\\
        gradients,\\
        KV blocks
...
\draw[brace, thick, color=consumerTeal] ([xshift=-0.4cm]hw.west |- hw.south) -- ([xshift=-0.4cm]rdma.west |- rdma.north)
    node[midway, left=6pt, font=\scriptsize, text=consumerTeal, align=right] {Driver\\nightmares};

\draw[brace, thick, color=enterpriseGold] ([xshift=-0.4cm]ucx.west |- ucx.south) -- ([xshift=-0.4cm]nccl.west |- nccl.north)
    node[midway, left=6pt, font=\scriptsize, text=enterpriseGold, align=right] {Static\\topology};

\draw[brace, thick, color=middlewareRed] ([xshift=-0.4cm]fw.west |- fw.south) -- ([xshift=-0.4cm]fw.west |- fw.north)
    node[midway, left=6pt, font=\scriptsize, text=middlewareRed, align=right] {No tensor\\awareness};

\end{tikzpicture}%
}
\caption{The complete GPU communication stack for distributed LLM systems. Each layer introduces static decisions that cannot adapt to runtime tensor access patterns. The proposed middleware (dashed) sits between the framework and application layers to provide adaptive routing.}
\label{fig:gpu-stack}
\end{figure}

\subsection{Layer 0--1: Hardware \& RDMA Drivers}

The foundation of GPU communication is inherently heterogeneous. A single system may combine NVLink (intra-node GPU-to-GPU), PCIe (CPU--GPU), and InfiniBand or Ethernet (inter-node), each with different latency characteristics, bandwidth limits, and driver requirements.

\textbf{Key issues at this layer:}

\begin{itemize}[leftmargin=1.5em, itemsep=3pt]
\item \textbf{GPUDirect RDMA failures on consumer GPUs.} GPUDirect requires specific driver support (nvidia-peermem kernel module). Consumer GPUs (GeForce series) frequently lack proper support, causing silent fallback to host-staged copies that double latency.

\item \textbf{Driver version fragility.} MOFED (Mellanox OFED) vs.\ upstream rdma-core mismatches cause 6$\times$ performance drops, as documented in UCX issue \#4770. On consumer hardware without Mellanox NICs, using SoftRoCE or iWARP through Intel X710 introduces an entirely different driver surface.

\item \textbf{NUMA topology misalignment.} When GPUs, NICs, and CPU sockets are on different NUMA domains, cross-socket memory access adds 30--50\% latency. DGX systems have carefully mapped affinity; consumer builds rarely do.
\end{itemize}

\subsection{Layer 2: UCX --- Transport Abstraction}

UCX~\cite{ucx} provides a unified API across InfiniBand, RoCE, TCP, shared memory, and NVLink. It is the transport layer beneath OpenMPI and is increasingly used by ML frameworks (vLLM uses UCX via NIXL for disaggregated serving).

\textbf{Key issues:}

\begin{itemize}[leftmargin=1.5em, itemsep=3pt]
\item \textbf{Static transport selection.} UCX selects the transport at connection setup and does not adapt based on message size or access frequency. A 16-byte KV cache pointer and a 500MB gradient tensor traverse the same path.

\item \textbf{Memory management side effects.} Mistral AI discovered a production memory leak in vLLM's disaggregated serving caused by UCX's mmap hooking mechanism for RDMA memory registration. UCX hooks \texttt{mmap}/\texttt{munmap} system-wide to optimize RDMA transfers, but in vLLM's use case (one large contiguous KV cache region), this created unbounded RSS growth.

\item \textbf{Consumer hardware compatibility.} UCX's GPU RDMA path requires specific rdma-core versions ($\geq$28.0) and compatible OFED installations. Consumer setups frequently hit version mismatches, causing fallback to slower transport modes without warning.
\end{itemize}

\subsection{Layer 3: NCCL / RCCL --- Collective Communication}

NCCL~\cite{nccl} is the de facto standard for GPU collective operations in deep learning. It implements topology-aware ring and tree algorithms for AllReduce, AllGather, Broadcast, and point-to-point operations. RCCL is AMD's equivalent for ROCm GPUs.

\textbf{Key issues:}

\begin{itemize}[leftmargin=1.5em, itemsep=3pt]
\item \textbf{Static topology at init, no runtime adaptation.} NCCL discovers hardware topology once during \texttt{ncclCommInitRank()} and selects ring or tree algorithms accordingly. During a 72-hour training run, as gradient magnitudes shift and attention patterns change, the communication strategy remains frozen.

\item \textbf{No message-size differentiation.} A 64-byte KV cache metadata pointer and a 2GB activation tensor shard are routed through identical channels. There is no priority mechanism to ensure latency-sensitive small transfers complete before bulk data.

\item \textbf{NVIDIA lock-in.} RCCL (AMD) is API-compatible but has different performance characteristics and bugs. Cross-vendor deployments must maintain separate tuning profiles.
\end{itemize}

\subsection{Layer 4: ML Frameworks --- Parallelism Strategies}

Parallelism strategies in vLLM---Tensor Parallelism (AllReduce after every layer, high frequency, latency-critical), Pipeline Parallelism (Send/Recv between stages, pipeline bubble overhead), and Expert Parallelism (AllToAll for token routing, dynamic, input-dependent)---have fundamentally different communication requirements. None of these strategies distinguish tensor access frequency at the transport level: KV cache heads, gradient accumulations, and weight shards share the same NCCL channels.

\subsection{Consumer vs.\ Enterprise: Divergent Failure Modes}

The communication stack fails differently for consumer and enterprise deployments. Consumer environments face: no GPUDirect RDMA (GeForce drivers lack nvidia-peermem), mismatched NICs (iWARP, basic RoCE, or plain TCP), no MOFED/OFED (must use upstream rdma-core or SoftRoCE), NUMA chaos (GPUs and NICs on random PCIe slots), mixed GPU generations, and shared machines where kernel modules cannot be installed.

Enterprise environments face: NCCL deadlock risk (mixing NCCL + MPI collectives), UCX memory leaks (mmap hooking causes unbounded RSS in disaggregated serving), static NCCL topology (cannot adapt to workload phase shifts during multi-day training runs), vendor lock-in (NCCL$\to$RCCL migration requires retuning), MoE routing overhead, and profiling gaps.

\textbf{The shared root cause:} no layer in the stack adapts to tensor access patterns at runtime. An adaptive middleware layer addresses both.

\subsection{The Head-of-Line Blocking Problem}

In distributed LLM inference, small latency-sensitive data (KV cache blocks, attention metadata, token embeddings) gets queued behind large bulk transfers (gradient accumulations, weight shards, activation tensors). This creates head-of-line blocking that directly inflates Time-To-First-Token (TTFT). The proposed adaptive dual-channel routing uses a WFA-based classifier to dynamically assign tensors to dedicated RDMA queue pairs: HOT tensors (KV cache, attention) get a priority QP with polled completions, while BULK tensors (gradients, activations) use a throughput-optimized channel. This separation reduces TTFT by an estimated 3--5$\times$.

\subsection{Scaling Projections}

As models grow from 7B to 70B to 405B to MoE 671B parameters, the number of GPUs required scales from 1--2 to 32--128, KV cache per request scales from 0.5--2~GB to 8--64~GB, and inter-node bandwidth requirements scale from $<$1~GB/s to 200+~GB/s. Communication overhead grows superlinearly while NIC bandwidth grows linearly at best. The gap must be closed by smarter routing, not just faster hardware.

\subsection{Where the Middleware Fits}

The three-module middleware architecture---Telemetry Monitor (tracks tensor access frequency, size, latency per QP), WFA Classifier (assigns hot/warm/cold labels), and Smart Router (maps classifications to dedicated RDMA queue pairs)---sits above verbs/UCX and below NCCL/framework layers. A feedback loop from QP performance back to the telemetry monitor enables continuous adaptation, which is the key differentiator from NCCL's static approach. This is the first system to jointly optimize both training communication (gradient routing) and inference communication (KV cache placement) using adaptive runtime telemetry, on consumer hardware, without kernel modifications or enterprise networking equipment.

%=============================================================================
\section{Work-First Scheduling for Tensor Transport}
\label{sec:work-first}
%=============================================================================

The Work-First Algorithm (WFA) originates from Frigo, Leiserson, and Randall's implementation of Cilk-5~\cite{frigo1998cilk5}: \emph{minimize the scheduling overhead borne by the work of a computation, concentrating on reducing overheads that contribute to $T_1$ even at the expense of overheads that contribute to $T_\infty$.}

In a computation with work $T_1$ (total operations) and span $T_\infty$ (critical path length), the expected runtime on $P$ processors under work-stealing is:
\begin{equation}
...
The work-first principle mandates \textbf{continuation stealing}. On a \texttt{spawn}, the spawning worker immediately executes the child (the ``work'') and pushes the continuation onto the deque for potential stealing. Continuation stealing guarantees $O(S_1 \cdot P)$ space bound~\cite{blumofe1999workstealing}---$P$ processors use at most $P$ times the single-processor stack space. Child stealing provides no such guarantee.

Guo et al.~\cite{guo2009workfirst} formalized the distinction between work-first and help-first policies: work-first wins for deep recursion with good temporal locality, help-first wins for broad fork-join with many short-lived tasks. SLAW~\cite{guo2010slaw} selects the policy per-task at runtime based on locality hints---an adaptive approach that directly parallels our WFA tensor classifier.

\subsection{Cache and NUMA Implications}
...
\begin{equation}
    ...
    \label{eq:cachemiss}
\end{equation}
where $Q_1$ is the serial cache complexity, $M$ is cache size, and $B$ is cache line size. Work-first minimizes this penalty because depth-first execution preserves serial locality, steals are rare ($O(P \cdot T_\infty)$ total), and stolen tasks are large (amortizing cache refill cost).

Standard randomized work-stealing is locality-oblivious. SLAW~\cite{guo2010slaw} addresses this by grouping workers into ``places'' aligned with NUMA topology, preferentially stealing within the same socket before attempting remote steals.

\subsection{Mapping to RDMA Tensor Transport}

The work-first principle translates directly to tensor transport: \emph{minimize overhead on the computation-critical path, even at the expense of the transfer coordination path}. The WFA classifier implements this:

\begin{itemize}
    \item \textbf{HOT tensors} (``fast clone''): Dedicated QP, polled completions, pinned memory---minimal per-transfer overhead on the critical path. Total overhead $\sim$2~$\mu$s.
    \item \textbf{WARM tensors}: Shared QP, event-driven completions. Moderate overhead.
    \item \textbf{COLD tensors} (``slow clone''): Batched transfers, compression enabled---higher per-transfer overhead, but off the critical path. Total overhead $\sim$25~$\mu$s.
\end{itemize}

The 12$\times$ difference between fast and slow paths justifies the WFA classifier's resource allocation: dedicating QPs and pre-registered buffers to HOT tensors is the work-first principle in action.

Shiina and Taura~\cite{shiina2022continuation} demonstrated distributed continuation stealing over MPI-3 RMA (one-sided RDMA), confirming that continuation stealing outperforms child stealing in distributed settings where the cost of remote memory access is amplified. The $O(S_1 \cdot P)$ space bound prevents RDMA buffer exhaustion---under child stealing, unbounded task queues translate to unbounded memory registration, exhausting NIC resources.

\subsection{Scheduling Policy Selection}

SLAW's adaptive policy selection suggests the WFA classifier should not use a single fixed strategy:

\begin{table}[htbp]
\centering
\caption{Tensor type to scheduling policy mapping based on work-first analysis}
\label{tab:wf-policy}
\begin{tabular}{@{}llll@{}}
\toprule
...
The work-first principle provides the theoretical foundation for the hot/warm/cold classification: it is an instance of the work-first principle applied to a memory hierarchy that extends from CPU cache through GPU memory to RDMA network transfers. The thresholds $\theta_1$ and $\theta_2$ correspond to the boundary between common case (local, fast) and rare case (remote, slow) in the work-stealing framework.

%=============================================================================
% THESIS BODY --- Sections continued from Updates 2--10
%=============================================================================
...
\begin{enumerate}[label=\textbf{C\arabic*.}]
    ...
    \item \textbf{RDMA Tensor Cache:} A centralized precision management layer in the tensor database that resolves FP16/BF16 mismatches across heterogeneous GPUs.
...
The \texttt{RDMAKVConnector} extends this further by pushing KV caches to the tensor database after prefill and pulling them for decode, backed by the \texttt{RDMATensorCacheMiddleware} which handles precision conversion and transport selection transparently.
...
\subsection{Tensor Database Architecture}
\label{sec:database-architecture}

The \texttt{tensor\_rdma} crate~\cite{scuffedrdma-repo} is a workspace member in the tensor database runtime. It depends on \texttt{tensor\_store} for local key-value storage and \texttt{tensor\_compress} for delta encoding.
...
\caption{Tensor database \texttt{tensor\_rdma} crate dependency graph. Six internal modules implement the RDMA Tensor Cache.}
...
%=============================================================================
\section{Test 1: UCX and SoftRoCE Cross-Node Validation}
\label{sec:test1-ucx-softroce}
%=============================================================================

This section documents the first empirical test of RDMA communication between the Phase~1 cluster nodes (Cerberus and Chimera), including hardware reconnaissance, software installation, patches required, and performance results.

\subsection{Hardware RDMA Asymmetry}
\label{sec:test1-hardware}

A critical discovery during hardware bring-up was that the two GPU nodes have fundamentally different RDMA capabilities:

\begin{table}[htbp]
\centering
\caption{Phase 1 node RDMA hardware comparison}
\label{tab:rdma-hardware-comparison}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Property} & \textbf{Cerberus} & \textbf{Chimera} \\
\midrule
NIC & Intel X710-DA2 (dual-port 10 GbE) & Aquantia AQC107 (10 GbE) \\
Driver & \texttt{i40e} + \texttt{irdma} & \texttt{atlantic} \\
Native RDMA & iWARP (via \texttt{irdma} module) & \textbf{None} \\
SR-IOV & 64 VFs per port & 7 VFs (Realtek 2.5 GbE only) \\
Connected NIC & Realtek 2.5 GbE (\texttt{enp71s0}) & Realtek 2.5 GbE (\texttt{enp71s0}) \\
GPUs & 2$\times$ RTX 5090 (32 GB each) & 3$\times$ RTX 3090 (24 GB each) \\
CUDA & 12.8 & 12.4 \\
Kernel & 6.17.0-14-generic & 6.8.0-63-generic \\
\bottomrule
\end{tabular}
\end{table}

The Intel X710 supports iWARP natively through the \texttt{irdma} kernel module (loaded automatically), but the Aquantia AQC107 on Chimera provides \emph{zero} RDMA capability. Furthermore, iWARP and RoCEv2 are not interoperable---even if both NICs supported RDMA, the protocol mismatch would prevent communication. This asymmetry motivates the use of \textbf{SoftRoCE} (\texttt{rdma\_rxe}) on both nodes to provide a uniform RoCEv2 transport layer over standard Ethernet.

\subsection{SoftRoCE Configuration}
\label{sec:test1-softroce-setup}

SoftRoCE was configured on both nodes by loading the \texttt{rdma\_rxe} kernel module and binding to the IP-bearing Ethernet interfaces:

\begin{lstlisting}[language=bash,caption={SoftRoCE setup on both nodes}]
# Both nodes: load SoftRoCE module
sudo modprobe rdma_rxe

# Cerberus: bind to eno2np1 (192.168.1.233)
sudo rdma link add rxe0 type rxe netdev eno2np1

# Chimera: bind to enp71s0 (192.168.1.150)
sudo rdma link add rxe0 type rxe netdev enp71s0
\end{lstlisting}

\textbf{Interface selection issue:} The initial configuration bound SoftRoCE to the 10~GbE ports (\texttt{eno1np0} on Cerberus, \texttt{enp69s0} on Chimera), which lacked IPv4 addresses. RDMA verbs require routable IP addresses for GID resolution. The fix was to rebind to the Realtek 2.5~GbE interfaces that carry the \texttt{192.168.1.0/24} subnet, reducing maximum link speed to 2.5~Gbps but enabling end-to-end connectivity.

\subsection{UCX Build and Patches}
\label{sec:test1-ucx-build}

UCX v1.17.0 was built from source on both nodes with CUDA, verbs, and RDMA-CM support:

\begin{lstlisting}[language=bash,caption={UCX v1.17.0 build configuration}]
...
./configure --prefix=/usr/local/ucx \
  --with-cuda=/usr/lib/cuda \
  --with-verbs --with-rdmacm \
  --enable-mt --enable-optimizations
make -j$(nproc) && sudo make install
\end{lstlisting}

UCX detected the following transport modules on both nodes:
\begin{itemize}
    \item \texttt{UCT modules: cuda ib rdmacm cma}
    \item \texttt{UCM modules: cuda}
    \item \texttt{Transports: rc\_verbs, ud\_verbs, tcp, cuda\_copy, cuda\_ipc}
\end{itemize}

\paragraph{Patch 1: Chimera \texttt{libnl} version mismatch.}
Chimera's \texttt{libnl-3-200} package had been updated to \texttt{3.7.0-0.3build1.1} while the development headers (\texttt{libnl-3-dev}) required exactly \texttt{3.7.0-0.3build1}. This prevented installation of \texttt{libibverbs-dev} and \texttt{librdmacm-dev}. The fix was to downgrade the runtime library:
\begin{lstlisting}[language=bash,caption={libnl version fix on Chimera}]
sudo apt-get install -y --allow-downgrades \
  libnl-3-200=3.7.0-0.3build1 \
  libnl-route-3-200=3.7.0-0.3build1
\end{lstlisting}

\paragraph{Patch 2: Java bindings JDK mismatch.}
Chimera had OpenJDK 21 runtime installed alongside JDK 17 development tools. The Maven javadoc plugin detected the Java~21 JRE but could not locate \texttt{/usr/lib/jvm/java-21-openjdk-amd64/bin/javadoc} (only JDK~17's tools were present). The workaround was to build UCX with \texttt{--without-java}.

\paragraph{Patch 3: Kubernetes overlay network interference.}
Both nodes run Kubernetes with Calico and Flannel networking, which inject overlay interfaces (\texttt{flannel.1} at \texttt{10.42.x.0/32}, \texttt{cali*} interfaces). UCX's transport selection algorithm selected the Flannel overlay for data transfer, causing connection timeouts:
\begin{lstlisting}[basicstyle=\ttfamily\footnotesize,caption={UCX error from Flannel route selection}]
UCX ERROR connect(fd=48, dest_addr=10.42.2.0:51941)
  failed: Connection timed out
UCX ERROR ucp_ep_create() failed: Destination is unreachable
\end{lstlisting}
The fix was to restrict UCX to the physical network interface via:
\begin{lstlisting}[language=bash,caption={UCX network device restriction}]
# Cerberus
export UCX_NET_DEVICES=eno2np1

# Chimera
export UCX_NET_DEVICES=enp71s0
\end{lstlisting}

\subsection{Test Results}
\label{sec:test1-results}

After applying all patches, both RDMA verbs and UCX transport tests completed successfully:

\begin{table}[htbp]
\centering
\caption{Test 1: Cross-node RDMA and UCX performance (Cerberus $\leftrightarrow$ Chimera)}
\label{tab:test1-results}
\begin{tabular}{@{}llrl@{}}
\toprule
\textbf{Test} & \textbf{Transport} & \textbf{Result} & \textbf{Notes} \\
\midrule
\texttt{ib\_write\_bw} & SoftRoCE (rxe0) & 0.91 Gbps & 65536-byte messages, 5000 iters \\
UCX \texttt{tag\_bw} (8~B) & TCP/verbs & 2.15 MB/s & Small-message latency bound \\
UCX \texttt{tag\_bw} (1~MB) & TCP/verbs & 111.86 MB/s & $\approx$0.89 Gbps sustained \\
\midrule
\multicolumn{2}{@{}l}{\textit{Theoretical max (Realtek 2.5 GbE)}} & \textit{2.5 Gbps} & \textit{$\approx$295 MB/s} \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Analysis.}
Both \texttt{ib\_write\_bw} and UCX achieve approximately 36--38\% of the theoretical 2.5~GbE line rate. The gap is attributable to SoftRoCE CPU overhead (software RDMA emulation adds per-packet processing), TCP/IP stack overhead (SoftRoCE encapsulates verbs operations in UDP packets), and Calico/iptables processing on both nodes. The Kubernetes networking stack imposes a non-trivial overhead through iptables rule traversal even for traffic that ultimately exits via the physical interface.

\paragraph{Key findings.}
\begin{enumerate}
    \item \textbf{RDMA hardware asymmetry} is a real deployment issue: two nodes in the same cluster had incompatible RDMA stacks (iWARP vs.\ none), requiring SoftRoCE as a lowest-common-denominator solution.
    \item \textbf{Kubernetes overlay interference} with UCX transport selection is an undocumented pitfall. The \texttt{UCX\_NET\_DEVICES} environment variable is essential in any K8s-hosted RDMA deployment.
    \item \textbf{Interface binding} must target IP-bearing interfaces; SoftRoCE requires routable addresses for GID-based endpoint resolution.
    \item The \textbf{2.5~GbE bottleneck} limits practical throughput regardless of SoftRoCE or UCX tuning. The 10~GbE NICs (X710 and Aquantia) require direct cabling to unlock their full bandwidth.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{V100 pair (Tower 2):} Forward and backward passes in FP16 Tensor Cores. Gradients transferred to the tensor database via RDMA.
...
Feature steering modifies behavior by clamping features: $\mathbf{f}'_i = \alpha \cdot \mathbf{f}_{\max,i}$. This requires no gradient computation and no backward pass---only a sparse vector read from the tensor database. SAE feature vectors map to the \texttt{SparseVector} type with $\sim$655$\times$ compression (a 65536-dim vector with $\sim$50 active features: 256~KB dense $\to$ $\sim$400~bytes sparse, storing only non-zero index--value pairs).
...
P8 & Tensor Database & \texttt{tensor\_rdma} Rust crate (116 tests, clean): precision, rounding, prefetch, transport, cache, config & \S\ref{sec:rdma-cache} \\
\bottomrule
\end{tabular}%
}
\end{table}
...
Projects P9--P11 can proceed in parallel. P12 depends on P8 (tensor database). P13--P14 can proceed independently. P15 requires P9--P13. P16 runs continuously alongside all projects.

%=============================================================================
\section{Per-Architecture Transport Extension}
\label{sec:arch-ext}
%=============================================================================

This section extends the transport design beyond the Transformer
KV-cache case that Updates~2--5 focused on. Three 2026 results
change the architectural picture materially: FlashAttention-4 for
Blackwell~\cite{zadouri2026flashattention4}, Mamba-3's reduced-state
SSM~\cite{lahoti2026mamba3}, and SonicMoE's IO-aware MoE
kernels~\cite{guo2026sonicmoe}. Each reshapes the per-layer state
memory that a disaggregated transport layer has to move. The
classify-then-route primitive in libscuffedrdma (WFA classifier plus
PMP controller) generalises to all three; the $O(1)$ SSM case is a
degenerate collapse, not a departure.

\subsection{Architecture-specific state memory}
\label{sec:arch-mem}

Table~\ref{tab:arch-mem-budget} gives the per-token state memory
that a disaggregated transport must carry. The comparison draws on
the self-attention complexity table in the \emph{Dive into Deep
Learning} textbook~\cite{zhang2023d2l}: self-attention has
$O(n^2 d)$ compute with $O(1)$ sequential ops and $O(1)$ maximum
path length; an RNN-shaped recurrence has $O(n d^2)$ compute,
$O(n)$ sequential ops, and $O(1)$ state. Mamba-3 rehabilitates the
RNN column by making the associative-scan parallelisable while
keeping the $O(1)$ state.

\begin{table}[htbp]
\centering
\caption{State memory, growth law, and dominant transport pattern
per architecture family.}
\label{tab:arch-mem-budget}
\small
\begin{tabular}{lllll}
\toprule
Family        & State/token             & Growth w/ ctx & Traffic     & Hot/cold bias \\
\midrule
Transformer   & $2 \cdot n_h \cdot d_h$ & $O(N)$        & bulk KV     & cold (bulk)   \\
Mamba-3       & $d_{\text{state}} \cdot d_{\text{model}}$ & $O(1)$  & small-state & hot (latency) \\
MoE           & $2 \cdot k \cdot d_{\text{model}}$ & $O(N)$ & all-to-all & cold + hot split \\
Hybrid        & mix                     & mixed         & mixed       & per-layer     \\
\bottomrule
\end{tabular}
\end{table}

At 128k context a 7B dense Transformer holds on the order of 16\,GB
of KV per user; a 1B Mamba-3 holds a fixed hidden state on the order
of 13\,MB independent of context. Three orders of magnitude of
difference invert which term of $T_{\text{KV transfer}} =
(\text{bytes}/W_{\text{net}}) + T_{\text{queuing}}$ dominates. CoPE's
``free-lunch'' context extension~\cite{li2026cope} --- clipping
RoPE frequencies so a 4k-trained model serves 128k with no retraining
--- amplifies the Transformer case specifically: it does not shrink
the KV growth law, it just lets production workloads reach the point
where that growth law bites.

\subsection{scuffedQuant extended to SSM state and MoE activations}
\label{sec:sq-ext}

The PolarQuant + QJL pipeline~\cite{zandieh2025turboquant} is
architecture-agnostic in the sense that its only input requirement
is that per-coordinate post-rotation distributions are approximately
Gaussian. That property holds for KV cache vectors, for SSM hidden
states (selective-scan produces low-rank but full-support states),
and for routed MoE expert activations (dense along $d_{\text{model}}$
even when sparse along the expert axis). We extend the existing
\texttt{ScuffedQuant} class with two wrappers,
\texttt{compress\_ssm\_state} and \texttt{compress\_expert\_activation},
that reshape into $(n, d)$ and delegate to the existing
Walsh-Hadamard + Lloyd-Max + QJL path. The math is unchanged.

Table~\ref{tab:sq-ext-results} reports cross-node results from the
\texttt{benchmarks/test\_arch/} harness, run on Chimera
(3$\times$~RTX~3090, Ampere) and Cerberus (2$\times$~RTX~5090,
Blackwell). The \texttt{compression\_ratio} column is identical
across nodes because the quantizer is deterministic NumPy; small
variations in \texttt{top-8} rank overlap are floating-point
non-associativity in the two GPUs' bfloat16 forward passes producing
slightly different inputs to the quantizer.

\begin{table}[htbp]
\centering
\caption{scuffedQuant extended: compression ratio and top-8 rank
overlap at 3-bit across four architecture families.}
\label{tab:sq-ext-results}
\small
\begin{tabular}{lrrr}
\toprule
Architecture (model)                           & Ratio (3-bit) & Chimera (3090) & Cerberus (5090) \\
\midrule
Transformer (Granite 3.3-2B, 4 layers)         & 4.0$\times$   & 82.4\%         & 83.6\%          \\
Mamba (state-spaces/mamba-130m-hf, fallback)   & 1.39$\times$  & 97.4\%         & 97.4\%          \\
Granite 4 hybrid (h-micro)                     & 5.6$\times$   & 95.9\%         & 96.3\%          \\
Granite 4 MoE expert activation (h-tiny)       & 7.6$\times$   & 100.0\%        & 100.0\%         \\
\bottomrule
\end{tabular}
\end{table}

Two observations. First, the SSM row's 1.39$\times$ ratio is not a
quality failure (top-8 stays $\geq 97\%$) but a geometry consequence:
the per-row overhead (norms, QJL signs, residual norms) amortises
poorly when $d_{\text{state}}$ is small (16 here). For a wider SSM
the ratio rises. Second, the MoE activation row's 7.6$\times$ ratio
at 100\% top-8 is the strongest case for compressing before the
all-to-all shuffle --- the bulk path shrinks nearly an order of
magnitude with no quality loss on the captured activation. This
aligns with the SonicMoE direction~\cite{guo2026sonicmoe}: the
expert-dispatch bottleneck is bandwidth-dominated, so any per-token
byte reduction composes with their IO/tile-aware kernel gains.
Mamba-3 is cited as the Mamba direction the Mamba v1 fallback
stands in for when \texttt{state-spaces/mamba-3-1b} becomes
public~\cite{lahoti2026mamba3}; the transport implication
(state size $\downarrow$, hot-QP dominance) is independent of
which Mamba version actually ships. Mamba-2 established the
structured state-space duality that both versions build on~\cite{dao2024mamba2}.

\subsection{GPU decompression and autotune experiment}
\label{sec:gpu-decompress}

The decode side of the pipeline must reconstruct FP32 tensors from
the compressed bundle before attention reads them. We ported the
inverse Walsh-Hadamard + codebook gather + sign multiply into a
single matmul using a precomputed Hadamard matrix
($H\,H^\top = I$), yielding bit-identical output to the CPU
reference and enabling \texttt{torch.compile} autotune.
Table~\ref{tab:gpu-decompress-ratio} shows the cross-GPU result
on a synthetic workload (128-dim vectors, 3-bit quantization). The
Blackwell/Ampere speedup is 1.6$\times$ at 131k rows and widens to
roughly 2$\times$ at 4k rows where launch and shared-memory
throughput dominate over memory bandwidth.

\begin{table}[htbp]
\centering
\caption{scuffedQuant GPU decompression latency: Ampere vs.\
Blackwell.}
\label{tab:gpu-decompress-ratio}
\small
\begin{tabular}{rrrrr}
\toprule
Rows    & Bits & 3090 ($\mu$s) & 5090 ($\mu$s) & 5090 / 3090 \\
\midrule
4\,096   & 3 & 994   & 511   & 1.95$\times$ \\
32\,768  & 3 & 4\,475  & 2\,500  & 1.79$\times$ \\
131\,072 & 3 & 34\,759 & 21\,788 & 1.60$\times$ \\
\bottomrule
\end{tabular}
\end{table}

A natural follow-up --- in the spirit of the ScuffedKernels
sub-project from Update~3 --- is to let \texttt{torch.compile}
autotune the fused kernel. The result is negative and informative:
on both GPUs the eager path wins by 3--6\%, because the decomposition
is memory-bound on the \texttt{codebook[indices]} gather rather than
compute-bound on the Hadamard matmul. PyTorch's eager dispatcher
already calls cuBLAS for the matmul and fuses the elementwise tail,
so Inductor's autotuned Triton variant finds essentially the same
kernel. Any further improvement over eager would need a hand-written
Triton kernel that fuses gather$\to$matmul$\to$rescale in one pass
and avoids materialising the $(n, d_{\text{pad}})$ intermediate;
that is the real ScuffedKernels extension, not a compile flag.
FlashAttention-4's asymmetric pipelining~\cite{zadouri2026flashattention4}
is evidence that the gains on Blackwell come from hand-written kernel
design, not from recompilation of existing torch ops.

\subsection{Relative position as a transport signal}
\label{sec:pos-as-signal}

Self-attention has no intrinsic notion of token order; order is
injected through positional encoding. The canonical sinusoidal
encoding~\cite{zhang2023d2l}
\[
  p_{i,2j} = \sin(i/10000^{2j/d}), \qquad
  p_{i,2j+1} = \cos(i/10000^{2j/d})
\]
encodes coarse position at low frequencies, fine position at high
frequencies, and relative offsets as linear transforms in that
space. Modern LLMs use RoPE rather than raw sinusoidal PE; CoPE's
frequency-clipping~\cite{li2026cope} sharpens the near-vs-far
contrast at the band the classifier actually samples. This same
signal is a \emph{transport} signal, not just a modelling signal.

Three regimes:
\begin{itemize}
\item \textbf{Fresh tokens} (sequence tail): read on every decode
step. Route to the hot QP, minimal compression.
\item \textbf{Mid-sequence tokens}: read with decaying probability.
Route to the cold QP with aggressive (3-bit) compression; if the
decode step does not read them, the link saved a trip.
\item \textbf{Attention-sink tokens} (first 0--3 positions): read on
every step regardless of absolute position, per the streaming-attention
literature. Route to the hot QP alongside fresh tokens. The
correct classifier input is \emph{predicted read frequency}, not raw
position --- sinks are the teaching case.
\end{itemize}

The WFA classifier extends with a relative-position feature
$\rho_t = (N-t)/N$ for token $t$ in context length $N$. The
hot-routing decision becomes
\[
  \mathrm{hot}(t) \;=\; \big[\alpha \rho_t + \beta \mathbb{1}_{t \in \text{sinks}} + \gamma s_t > \tau\big],
\]
where $s_t$ is the size-class term the classifier already uses. PMP
is unchanged: the switching function consumes the classifier's
output, it just gets a richer input. For Mamba-3 the feature
degenerates (no per-token state to place) and the classifier
collapses to ``always hot,'' which is the correct behaviour for an
$O(1)$-state model. For MoE the same feature applies, scored per
expert rather than per token: popular experts are the ``fresh'' row,
rare experts are the ``sink'' row. The MoE case is ground truth
rather than prior: the gating network tells the transport layer
directly which experts will read which tokens, whereas the
Transformer case requires inference.

\subsection{The classify-allocate primitive}
\label{sec:eml-primitive}

The four regimes above are parameter choices on a single primitive
pair: \emph{classify} (WFA on typed features) and \emph{allocate}
(PMP bang-bang over queue depth and service rate). Odrzyw{\'o}{\l}ek's
recent universal-operator construction~\cite{odrzywolek2026eml},
showing that exponentials and logarithms both reduce to the single
$\mathrm{eml}(x,y) = \exp(x) - \ln(y)$ operator, offers a useful
analogy: one compositional primitive; all the variety comes from
what you feed it. In the transport layer the analogous primitive is
$(\text{classify}, \text{allocate})$; Transformer KV, Mamba state,
and MoE activation are parameter choices, not separate
architectures. The implementation cost is correspondingly small:
two helper methods on the existing \texttt{ScuffedQuant} class
cover all three architecture families with no change to the core
math.

\subsection{Relation to 2025--2026 transport systems}
\label{sec:transport-related}

Three 2025--2026 systems target overlapping territory.
TransferEngine~\cite{chen2025transferengine} is a point-to-point
RDMA library portable across ConnectX-7 and AWS EFA, demonstrating
400\,Gbps peak bandwidth and sub-second RL weight updates for
trillion-parameter models. It is the datacenter-class analog of
libscuffedrdma's goal: a portable RDMA transport for LLM systems,
targeting enterprise hardware where libscuffedrdma targets consumer
10\,GbE. The libmesh-rdma project~\cite{libmesh_rdma} is the closest
sibling on the consumer side: an MIT-licensed 75\,KB library that
reaches 77\,GB/s and 11.6\,$\mu$s RTT on direct-connect ConnectX-7
DGX Spark systems without a managed switch, and whose TCP-based QP
bootstrap informed the hardened handshake in this thesis's Python
middleware. Where TransferEngine and libmesh-rdma provide the raw
transport, libscuffedrdma adds the tensor-aware classification
(WFA), bandwidth allocation (PMP), and compression (scuffedQuant)
layers above it --- features neither of the two transport libraries
implements.

The classify-then-route pattern has its own lineage. Zhou's 1987
dissertation on dynamic load balancing~\cite{zhou1987loadindex}
introduced the load-index abstraction as a typed per-host metric
consumed by a scheduler; that abstraction is the ancestor of every
``metric-driven router'' since, including LSF's LIM, IBM Spectrum
Symphony's ELIM, and the WFA classifier here. The novelty in
libscuffedrdma is not the classify-route pattern but the
granularity: the classifier sits below the service scheduler, in
the RDMA transport itself, where the dispatch budget is measured in
...
This report consolidates the research progress from Updates 2--12, presenting a unified view of the adaptive RDMA optimization framework for distributed LLM workloads. The key contributions are:
...
\begin{enumerate}
    ...
    \item \textbf{RDMA Tensor Cache}: A precision management layer in the tensor database that resolves heterogeneous GPU precision mismatches through FP32 master weight storage and stochastic rounding.
    ...
    \item \textbf{GPU communication stack analysis}: A layer-by-layer analysis of the full communication stack identifies head-of-line blocking as the core bottleneck and demonstrates how consumer and enterprise environments share the same root cause---lack of runtime tensor awareness.
    \item \textbf{Work-first scheduling theory}: The Cilk work-first principle provides theoretical grounding for the WFA classifier's hot/warm/cold tiers---minimizing overhead on the computation-critical path while accepting higher overhead for non-critical bulk transfers. SLAW's adaptive policy selection validates per-tensor strategy assignment.
\end{enumerate}
