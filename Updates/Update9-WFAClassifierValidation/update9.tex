...
This update validates the Weighted Finite Automaton (WFA) tensor classifier from the adaptive RDMA middleware against a real workload: LLM4Decompile, an open-source LLM that decompiles x86-64 binaries back into C source code. The WFA tensor classifier categorizes tensor access streams as hot, warm, or cold to drive RDMA Queue Pair routing decisions in the two-tower cluster. By instrumenting KV cache access patterns during decompilation inference on a Google Colab H100 GPU in BF16 with FlashAttention~2, we capture per-layer access frequencies and run the WFA classifier to categorize each layer's KV cache.
...
\begin{enumerate}
    ...
    \item \textbf{RISC-V assembly generation}: The same model architecture could be fine-tuned on RISC-V binaries using the heterogeneous training pipeline (V100 FP16 forward/backward, Neumann FP32 master weights, 5070~Ti BF16 validation), as described in the RISC-V assembly generation component of the thesis.
\end{enumerate}
...
This validates the thesis claim that SAE feature steering is a gradient-free alternative to fine-tuning and demonstrates that the WFA classifier correctly adapts its routing decisions when the inference workload changes. The SAE steering approach, as described in the thesis's discussion of Sparse Autoencoder feature manipulation, allows behavioral modification without backward passes or weight updates, making it particularly efficient for RDMA-connected inference where minimizing cross-tower traffic is critical.
...
\begin{enumerate}
    ...
    \item Generate the steered vs.\ unsteered comparison figure for inclusion in the thesis.
    ...
    \item Port the validated classifier and steering engine to the two-tower cluster for integration with the RDMA middleware.
    \item Explore fine-tuning LLM4Decompile on RISC-V assembly using the heterogeneous training pipeline.
\end{enumerate}
