...
\begin{enumerate}[noitemsep]
    ...
    \item \textbf{Fewer serving replicas at constant throughput.} By Little's Law ($L = \lambda W$), if each request completes faster ($W$ decreases), fewer concurrent model instances ($L$) are needed to sustain the same request rate ($\lambda$):
    \begin{align*}
        \text{Before:}\quad 10\;\text{req/s} \times 200\;\text{ms} &= 2.0\;\text{concurrent instances} \\
        \text{After (30\% TTFT reduction):}\quad 10\;\text{req/s} \times 140\;\text{ms} &= 1.4\;\text{concurrent instances}
    \end{align*}
    A 30--50\% TTFT reduction translates to roughly 23--33\% fewer serving replicas for the same SLA---a direct cost saving in production GPU clusters.
\end{enumerate}
