...
\title{
    ...
    \large Thesis Proposal --- A Unified Communication Framework Addressing\\Fragmentation Across NCCL, UCX, JACCL, and Emerging Standards
}
...
The core middleware is implemented as a Python class, \texttt{AdaptiveRDMAMiddleware}, whose constructor accepts a configuration object specifying WFA classifier parameters, telemetry settings, and transport preferences.
During initialization, the middleware instantiates a WFA classifier, a runtime telemetry collector, and a transport backend.
Transport initialization proceeds by first inspecting whether the process runs inside a container; if so, the shared-memory transport is excluded from the UCX transport list to avoid the isolation failures described in Issue~7.
The middleware then attempts to create a UCX-based transport as the primary backend and, upon failure, falls back to an NCCL-based transport.

The central method, \texttt{transfer\_tensor}, accepts a tensor object and a destination rank.
It queries the WFA classifier with the tensor's identifier, size, and historical access pattern obtained from the telemetry subsystem.
Depending on whether the tensor is classified as hot, warm, or cold, the method dispatches the transfer through a dedicated high-priority queue pair, a standard queue pair, or a batched transfer channel, respectively.
This design ensures that latency-critical tensors such as KV cache entries receive immediate, low-latency transport, while infrequently accessed buffers are coalesced for throughput efficiency.
