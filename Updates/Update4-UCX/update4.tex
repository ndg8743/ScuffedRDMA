...
\begin{enumerate}
\item Cross-node RDMA benchmark (Chimera to Cerberus, 10GbE) and multi-threaded concurrent workload to show head-of-line blocking elimination.
\item vLLM KvConnector integration with scuffedQuant compression in the transfer path.
\item Port pyverbs transport to Rust or C++ to remove the GIL bottleneck for concurrent QP polling.
\item Advisor alignment: PMP with Dr.\ Danesh, WFA with Dr.\ Suchy.
\end{enumerate}
