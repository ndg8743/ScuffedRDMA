...
This update introduces \textbf{tensor-aware RDMA traffic management}. The WFA classifier labels each transfer by its role in the inference pipeline. The PMP bang-bang controller allocates bandwidth between priority classes. Traffic routes to separate RDMA queue pairs: a hot pool for latency-sensitive transfers, a cold pool for throughput. We validate with real RDMA writes on SoftRoCE.
...
This section describes the architecture of libscuffedrdma, the thesis's RDMA middleware. The current prototype is written in Python using pyverbs (rdma-core bindings); a C++ port is a future production goal. Design decisions follow Austin et al.'s roofline framework and decode step time model~\cite{scaling_book}.
...
UCX is the dominant open-source RDMA transport framework, used by NCCL, UCC, MPI implementations, and distributed ML frameworks including PyTorch and Dask. The UCX issue tracker (\url{https://github.com/openucx/ucx/issues}) shows recurring problems caused by architectural decisions that libscuffedrdma avoids. This section maps those issues to the thesis contributions.
...
\begin{itemize}
    ...
    \item \textbf{Issue \#11091}: Upgrading from UCX 1.18.1 to 1.19.1 causes significant regressions due to changes in rendezvous performance difference calculations (PR \#10401), showing the RNDV heuristic is fragile across versions.
...
libscuffedrdma does not use message size to select a protocol. libscuffedrdma's WFA classifier uses tensor metadata--access frequency, inference phase, layer index, and inter-arrival time variance--to assign a priority class, and the PMP controller allocates QPs accordingly. A 64\,KB KV cache block for a decode step and a 64\,KB prefetch block for speculative prefill are the same size but need very different latencies; UCX treats them identically, libscuffedrdma does not.

\subsection{Head-of-Line Blocking in Shared QP Pools}

UCX uses a shared pool of RC queue pairs with static partitioning. A small eager message can queue behind a large rendezvous transfer on the same QP because QP selection ignores message priority. Several issues trace directly to this design:

\begin{itemize}
    \item \textbf{Issue \#11004}: All RoCE QPs across different processes use the \textbf{same UDP source port}, causing bond/LAG layer3+4 hashing to route all RDMA traffic through a single physical link. This happens because the shared QP pool does not differentiate flows. libscuffedrdma's per-classification QP pools (hot pool: 2--4 QPs, cold pool: 4--8 QPs) naturally produce different 5-tuple flow hashes, distributing traffic across bond members with no switch or bond configuration changes.
    \item \textbf{Issue \#11034}: Eight UCP workers sharing a \texttt{ucp\_context} contend on a \texttt{pthread\_spin\_lock} even when each operates in \texttt{THREAD\_MODE\_SINGLE}, preventing linear scaling. All workers share the same QP and CQ resources, causing contention. libscuffedrdma's dedicated QPs per traffic class eliminate this shared-resource bottleneck.
    \item \textbf{Issue \#1319}: A long-standing request to defer QP allocation until the first message, rather than at \texttt{ucp\_ep\_create} time. libscuffedrdma already implements this pattern: QPs are created per classification when the first tensor of that class is submitted, not at connection establishment.
\end{itemize}

\paragraph{The fundamental issue.}
UCX's QP pool is a shared resource managed by size-based heuristics. libscuffedrdma treats QPs as \emph{class-dedicated resources} managed by a control-theoretic policy (PMP). A hot-classified tensor always posts to the hot QP pool and never queues behind cold traffic. The \texttt{ibv\_post\_send} call targets a QP chosen by the WFA+PMP pipeline, not UCX's internal dispatcher.

\subsection{Transport Selection Opacity}

UCX's transport selection algorithm is a black box that users cannot inspect or override:
...
\begin{itemize}
    ...
    \item \textbf{Issue \#6511}: RC transport is prioritized over \texttt{dc\_x} (dynamically connected) even when DC is available and better suited. Users cannot control preference order.
    ...
    \item \textbf{Issue \#9560}: No fine-grained per-path transport pinning in multi-node environments. UCX selects different configurations depending on communication patterns, and users cannot pin specific transports per data path.
    \item \textbf{Issue \#10608}: Users cannot query which transport was actually used for a given transfer.
...
libscuffedrdma makes one transport decision: all inter-node tensor data goes through \texttt{ibv\_post\_send} on a QP selected by the WFA classifier. The mapping from tensor metadata to QP is deterministic given the WFA state and PMP signal $u^*(t)$ -- no opaque scoring or multi-transport negotiation. UCX's connection management (\texttt{rdma\_create\_id}, \texttt{rdma\_resolve\_addr}) handles only the initial handshake; libscuffedrdma controls the data path.

\subsection{Multi-Rail Failures}

UCX's multi-rail support--intended to aggregate bandwidth across multiple NICs--often fails to improve performance and can degrade it:
...
On the Cerberus/Chimera testbed (single 10\,GbE NIC per machine), multi-rail is not directly applicable. But the underlying issue -- NIC and NUMA awareness in QP placement -- is relevant: libscuffedrdma's direct \texttt{ibv\_post\_send} with NUMA-aware QP placement avoids the lane-selection logic that causes these failures. With a second NIC, hot QPs would be pinned to the NUMA-local NIC and cold QPs to the other, bypassing UCX's lane scoring.

\subsection{Rendezvous Protocol Bugs and Optimizations}

The rendezvous protocol--which libscuffedrdma bypasses for tensor data--has several open issues that reinforce this decision:

\begin{itemize}
    \item \textbf{Issue \#6721}: The first Active Message between an endpoint pair is \textbf{always received as eager} regardless of \texttt{UCX\_RNDV\_THRESH}, because endpoint wireup has not completed. Subsequent messages use rendezvous correctly. This means the first message always takes the eager path, skewing latency measurements and warmup.
    ...
    \item \textbf{Issues \#1298, \#1299, \#1300}: Missing optimizations for \texttt{get\_bcopy}, \texttt{put\_bcopy}, and receiver-side buffer registration in the rendezvous path. These have been open since 2017.
\end{itemize}

\paragraph{Why bypassing RNDV is justified.}
libscuffedrdma bypasses the rendezvous data path. It calls \texttt{ibv\_post\_send} directly with pre-registered memory regions. This eliminates the rendezvous handshake, avoids rail selection bugs, and removes the wakeup overhead. The trade-off is manual memory registration, but LLM tensor buffers are long-lived so registration cost is paid once at startup.
...
The middleware pre-registers KV cache buffers at allocation time using \texttt{ibv\_reg\_mr} directly, with explicit error checking (no silent fallback). Because vLLM's PagedAttention~\cite{vllm_paged} allocates KV cache blocks from a fixed pool, the registration set is fixed at startup. Registration cost is paid once, not per-transfer.
...
UCX was designed as a general-purpose communication framework and retrofitted for GPU/ML workloads, not built around them. libscuffedrdma is purpose-built for tensor transport: the WFA encodes knowledge of KV cache blocks, attention layers, and inference phases directly.
...
\paragraph{Relevance.}
Because libscuffedrdma calls \texttt{ibv\_post\_send} directly, it can set ECE bits and traffic class per QP at creation time (\texttt{ibv\_modify\_qp} with \texttt{IBV\_QP\_QKEY} and traffic class attributes). Hot QPs can be assigned a higher traffic class than cold QPs, enabling hardware-level prioritization on DSCP-capable switches. UCX's asymmetric TC bug (Issue \#10325) does not apply because libscuffedrdma controls both endpoints.
...
The UCX issue tracker shows these problems are real: they cause performance regressions, hangs, and incorrect behavior in production. The thesis does not fix UCX. For distributed LLM inference on consumer hardware, bypassing UCX's data path and using tensor-aware, control-theoretic QP management solves problems that UCX's configuration knobs cannot.
...
The key detail for RDMA: TurboQuant preserves \emph{inner product accuracy}, not individual vector accuracy. Since attention only needs inner products (query dot key), this suffices -- but the decode node needs a modified attention kernel that operates on quantized codes directly. The RDMA layer just moves the smaller compressed bytes.
...
Phase detection works by observing layer access order. Prefill sweeps sequentially through all layers (L0, L1, L2, ...). Decode repeatedly accesses the same layer as it generates tokens. The classifier detects the phase automatically -- no signal from the inference engine needed.
...
The deadband (0.1) prevents chattering: without it, the controller would flip between HOT and COLD every cycle when the queues are nearly balanced. The weights are asymmetric ($\alpha{=}2$, $\beta{=}1$) because draining the hot queue matters more -- hot traffic is latency-sensitive.

\subsection{How RDMA Testing Works on SoftRoCE}

SoftRoCE (\texttt{rdma\_rxe} kernel module) implements RDMA verbs in software over any Ethernet NIC. The kernel handles QP state, packet construction, and completions, so latencies are higher (tens of microseconds vs. single-digit). But the API is identical -- code that works on SoftRoCE runs on ConnectX hardware unchanged.
...
\begin{itemize}
\item pyverbs on Ubuntu 24.04 uses property setters for \texttt{QPAttr}, \texttt{set\_wr\_rdma(rkey, addr)} for RDMA work requests, and \texttt{cq.poll()} returns \texttt{(count, list)}. These differ from the rdma-core docs, so we found them by trial and error.
...
\item In synchronous single-threaded SoftRoCE, head-of-line blocking doesn't appear because each operation finishes before the next starts. The dual QP benefit requires concurrent producers or hardware CQ offload.
\end{itemize}
