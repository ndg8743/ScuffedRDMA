...
Update 1 covers the hardware testbed (Cerberus 2$\times$RTX 5090, Chimera 3$\times$RTX 3090, 10GbE interconnect). Update 2 covers the problem statement and initial architecture. Update 3 covers the tensor cache architecture, precision pipeline, and transport abstractions. This update builds on all three with the concrete implementation, PMP control theory, and benchmark results.

\subsection{The KV Cache Problem}

Without a KV cache, every decode step recomputes attention over the entire sequence--quadratic in sequence length. The KV cache makes decode linear: each new token computes its own key/value projections and attends over the cached values from all previous tokens.
...
This update introduces \textbf{tensor-aware RDMA traffic management}: classify each transfer by its role in the inference pipeline (WFA classifier), allocate bandwidth between priority classes (PMP bang-bang controller), and route traffic to separate RDMA queue pairs (hot pool for latency-sensitive, cold pool for throughput). This is validated with real RDMA writes on SoftRoCE.
