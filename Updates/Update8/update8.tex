...
\title{
    ...
    \large Update 8: RDMA Tensor Cache Architecture and Neumann Integration
}
...
\begin{abstract}
\noindent
Heterogeneous GPU clusters present a precision mismatch problem that fundamentally limits distributed LLM training and inference. This update presents the RDMA Tensor Cache, a centralized precision management layer implemented within the Neumann tensor database, designed for a two-tower home lab: Tower~1 (Ryzen~9~7950X, RTX~5070~Ti~16GB, Windows) and Tower~2 (Proxmox/Linux, $2\times$ Tesla~V100~32GB). The V100s lack BF16 support, forcing FP16 computation with its narrow dynamic range; the 5070~Ti supports BF16 natively but runs on Windows without kernel RDMA. We resolve this with an RDMA-accessible FP32 master weight store in Neumann, stochastic rounding for precision-lossy conversions, predictive tensor prefetching, and a centralized KV cache that eliminates the per-rank duplication problem inherent to Multi-Latent Attention (MLA) and Multi-Query Attention (MQA) under tensor parallelism. We detail the \texttt{tensor\_rdma} crate architecture, present three experimental pillars for validation, and describe integration paths through UCX/UCC, SoftRoCEv2, and the emerging Ultra Ethernet Consortium (UEC) 1.0 standard.
\end{abstract}
...
This update advances the thesis in three directions. First, we formalize the RDMA Tensor Cache architecture that bridges the precision gap between V100 and 5070~Ti GPUs by centralizing master weights in Neumann's \texttt{tensor\_rdma} crate. Second, we analyze the parallelism strategies available in vLLM and identify a structural limitation of tensor parallelism for MLA/MQA models that motivates our centralized KV cache design. Third, we introduce Sparse Auto-Encoders (SAEs) as an alternative to gradient-based fine-tuning, showing how SAE feature vectors map naturally to Neumann's \texttt{SparseVector} representation with 51$\times$ compression.
...
The fundamental tension is that the V100 is a datacenter GPU with full GPUDirect RDMA support and Tensor Core FP16 compute but no BF16, while the 5070~Ti is a consumer GPU with BF16 native support but no RDMA stack on Windows. The RDMA Tensor Cache resolves this asymmetry by placing FP32 master weights in Neumann, accessible from both towers through different transport paths.
...
GPUDirect RDMA~\cite{gpudirect-rdma-redhat} enables RDMA NICs to read/write GPU memory directly. It requires the \texttt{nvidia-peermem} kernel module, which registers GPU memory regions with the InfiniBand subsystem. On Tower~2 (Linux), this is the target configuration for V100 tensor transfers:

\begin{lstlisting}[language=bash, caption={GPUDirect RDMA setup on Tower 2}]
# Load nvidia-peermem for GPUDirect RDMA
modprobe nvidia-peermem

# Verify registration
dmesg | grep nv_mem
# nvidia-peermem: module loaded, peer memory registered
...
Megatron-LM's approach to mixed precision training is particularly relevant: it maintains FP32 master weights on the optimizer node while broadcasting FP16 copies to compute nodes. Our RDMA Tensor Cache generalizes this pattern by placing the FP32 master store in Neumann (accessible via RDMA) rather than requiring it to reside in GPU memory on a designated node. This decoupling enables the heterogeneous V100/5070~Ti configuration where no single GPU type can serve as the universal master.
...
Our solution is to externalize the KV cache to Neumann, accessed via RDMA. A single centralized copy serves all TP ranks through one-sided RDMA reads, eliminating duplication. Neumann's \texttt{rdma\_cache} module already provides the buffer registration and remote access semantics required (Section~\ref{sec:neumann-integration}).
...
\begin{tikzpicture}[
    ...
    neumann/.style={rectangle, draw, thick, rounded corners, fill=neumannpurple!15, minimum width=3cm, minimum height=0.8cm, align=center, font=\small},
...
\node[neumann] (neu) at (4.5, -0.6) {Neumann\\tensor\_rdma};
...
% Neumann detail
\node[draw=neumannpurple, fill=neumannpurple!5, rounded corners, below=2.8cm of $(t1)!0.5!(t2)$, minimum width=12cm, minimum height=1.6cm, align=center] (detail) {};
\node[above=0.05cm of detail.north, font=\small\bfseries\color{neumannpurple}] {Neumann RDMA Tensor Cache (FP32 Master Store)};
...
\draw[arrow, neumannpurple, dashed] (neu) -- (detail.north);

\end{tikzpicture}
\caption{Two-tower system architecture. Neumann's \texttt{tensor\_rdma} crate runs on Tower~2, serving as the centralized FP32 master weight store. Tower~1 accesses tensors via gRPC/TCP fallback; Tower~2's V100s access them via GPUDirect RDMA through \texttt{nvidia-peermem}.}
...
In the \texttt{tensor\_rdma} crate, stochastic rounding is implemented in \texttt{rounding.rs} using the \texttt{half} crate for FP16/BF16 manipulation and \texttt{rand} for probability generation. The \texttt{next\_fp16} and \texttt{next\_bf16} helper functions find adjacent representable values via bit manipulation:

\begin{lstlisting}[language=C, caption={Stochastic rounding implementation sketch (from rounding.rs)}, label={lst:stochastic-round}]
pub fn stochastic_round_f32_to_f16(value: f32, rng: &mut impl Rng) -> f16 {
    let floor = f16::from_f32(value);
    let floor_f32 = floor.to_f32();

    if (floor_f32 - value).abs() < f32::EPSILON {
        return floor; // Exact representation
    }

    let ceil = next_fp16(floor, value > floor_f32);
    let ceil_f32 = ceil.to_f32();
    let gap = ceil_f32 - floor_f32;
    let fraction = (value - floor_f32) / gap;

    if rng.gen::<f32>() < fraction { ceil } else { floor }
}
\end{lstlisting}

\subsection{Centralized KV Cache}

The MLA/MQA duplication problem identified in Section~\ref{sec:mla-problem} is resolved by storing the KV cache in Neumann's \texttt{RdmaTensorCache}, keyed by layer index and sequence position. Each TP rank issues one-sided RDMA reads to fetch the KV entries it needs for its attention head shard, rather than maintaining a local copy.
...
\begin{enumerate}
    ...
    \item Transport layer issues RDMA Read to Neumann's registered buffer for the predicted entries.
...
%=============================================================================
\section{Neumann Integration}
\label{sec:neumann-integration}
%=============================================================================

The \texttt{tensor\_rdma} crate~\cite{neumann-repo} is a workspace member in the Neumann unified tensor runtime. It depends on \texttt{tensor\_store} for local key-value storage and \texttt{tensor\_compress} for delta encoding. \Cref{fig:crate-deps} shows the dependency structure.
...
\end{tikzpicture}
\caption{Neumann \texttt{tensor\_rdma} crate dependency graph. The crate depends on \texttt{tensor\_store} for local caching and \texttt{tensor\_compress} for delta encoding. Six internal modules implement the RDMA Tensor Cache.}
...
The \texttt{DeviceCapability} struct encodes what each GPU supports. The \texttt{PrecisionRouter} uses these to select wire formats:

\begin{lstlisting}[language=C, caption={Device capabilities for our hardware}, label={lst:device-caps}]
// V100: FP16 native, FP32 supported, no BF16, GPUDirect RDMA
DeviceCapability::v100_32gb() -> DeviceCapability {
    name: "Tesla V100 32GB",
    supported_formats: [FP16, FP32],
    native_compute: FP16,
    vram_bytes: 34_359_738_368,
    gpu_direct_rdma: true,
}

// RTX 5070 Ti: BF16 native, FP16/FP32 supported, no GPUDirect
DeviceCapability::rtx5070ti() -> DeviceCapability {
    name: "RTX 5070 Ti 16GB",
    supported_formats: [FP16, BF16, FP32],
    native_compute: BF16,
...
\begin{itemize}
    \item \textbf{V100 pair (Tower 2):} Forward and backward passes in FP16 Tensor Cores. Gradients are produced in FP16 and transferred to Neumann via RDMA.
...
\subsubsection{Mapping to Neumann}

SAE feature vectors are a natural fit for Neumann's \texttt{SparseVector} type, which stores only non-zero indices and values:
\begin{lstlisting}[language=C, caption={SAE features as Neumann SparseVectors}]
// A 65536-dim SAE feature vector with ~50 active features
// Dense: 65536 * 4 bytes = 256 KB
// SparseVector: ~50 * (4 + 4) bytes = 400 bytes
// Compression: 51x (640:1 ratio)

let feature_vector = SparseVector::from_pairs(
    vec![(1204, 0.85), (3891, 0.42), (15023, 0.91), ...],
    65536, // total dimensions
);

// Store in Neumann for RDMA access
store.put("sae:layer_12:eiffel_tower", feature_vector);
\end{lstlisting}

The 51$\times$ compression from dense to sparse representation means SAE feature vectors are trivially transferable over RDMA---a single feature vector fits in a single RDMA read of $\sim$400~bytes, well below the MTU.

OpenAI's work on scaling SAEs~\cite{openai-sae-scaling} and the HuggingFace SAE collection~\cite{sae-collection} provide pre-trained SAEs for several open-weight models. These can be loaded into Neumann and served to both towers for inference-time steering.
...
The Python prototype bridges the \texttt{tensor\_rdma} Rust crate with vLLM's distributed inference engine. It follows the middleware architecture established in Update~6 and extends it with RDMA-native connectors.

\subsection{Architecture}

\begin{lstlisting}[language=Python, caption={RDMA Tensor Cache middleware extension}]
class RDMATensorCacheMiddleware:
    """Extends the ScuffedRDMA middleware with Neumann tensor cache access."""

    def __init__(self, config: MiddlewareConfig):
        self.transport = self._init_transport(config)
        self.neumann_client = NeumannClient(
            host=config.neumann_host,  # 192.168.2.111
            port=config.neumann_port,  # 4791
        )
        self.precision_router = PrecisionRouter(config.devices)

    def get_tensor(self, key: str, device: DeviceCapability) -> torch.Tensor:
        """Fetch tensor from Neumann with automatic precision conversion."""
        wire_format = self.precision_router.optimal_wire_format(device)
        raw = self.neumann_client.rdma_read(key)
    ...
    def put_gradient(self, key: str, gradient: torch.Tensor,
        ...
        """Send gradient to Neumann for FP32 master weight update."""
        wire_data = self._encode_from_torch(gradient, source_device)
        self.neumann_client.rdma_write(key, wire_data)

    def get_kv_cache(self, layer_idx: int, seq_positions: List[int]
                     ) -> torch.Tensor:
        """Fetch KV cache entries from centralized Neumann store."""
        keys = [f"kv:layer_{layer_idx}:pos_{p}" for p in seq_positions]
        return self.neumann_client.batch_read(keys)
\end{lstlisting}
...
class NeumannKVConnector:
    """vLLM KV cache connector backed by Neumann RDMA Tensor Cache."""
...
On Tower~2 (Proxmox/Linux), SoftRoCEv2 is configured through the \texttt{rdma\_rxe} kernel module:

\begin{lstlisting}[language=bash, caption={SoftRoCEv2 setup on Tower 2}]
# Load the software RDMA provider
modprobe rdma_rxe

# Create RXE device over the physical NIC
rdma link add rxe0 type rxe netdev ens3f0

# Verify
rdma link show
# link rxe0/1 state ACTIVE physical_state LINK_UP netdev ens3f0
...
For the V100 GPUs on Tower~2, GPUDirect RDMA provides the most efficient data path. With \texttt{nvidia-peermem} loaded, the RDMA NIC can read/write GPU memory directly:

\begin{lstlisting}[language=bash, caption={GPUDirect RDMA verification}]
# Load nvidia-peermem
modprobe nvidia-peermem

# Verify GPU BAR1 memory is accessible
...
# Test with perftest
ib_write_bw -d rxe0 --use_cuda=0  # Server (V100 GPU 0)
ib_write_bw -d rxe0 --use_cuda=0 192.168.2.111  # Client
\end{lstlisting}
...
The UCX environment variable \texttt{UCX\_TLS} can be used to constrain transport selection for debugging. Setting \texttt{UCX\_TLS=tcp} forces TCP on both sides; setting \texttt{UCX\_TLS=rc,tcp} allows the negotiation to select RC when available with TCP fallback. For our heterogeneous cluster, the recommended setting is:
\begin{lstlisting}[language=bash]
# Tower 2 (Linux, RDMA available)
export UCX_TLS=rc,dc,tcp,cuda_copy,gdr_copy
export UCX_NET_DEVICES=mlx5_0:1

# Tower 1 (Windows, TCP only)
export UCX_TLS=tcp
\end{lstlisting}
