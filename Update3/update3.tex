...
Soft-RoCE provides RDMA semantics over standard Ethernet without specialized hardware. Setup on both nodes:
...
\textbf{Recommendation}: Soft-RoCE is adequate for development/testing RDMA semantics but production deployments should use Hardware RoCE with Mellanox ConnectX NICs (expected latency $<$2$\mu$s).

\section{Tesla TTPoe Build}

Successfully compiled Tesla's Time-Triggered Protocol over Ethernet kernel modules:
...
\texttt{VLLM\_USE\_FLASHINFER\_MXFP4\_MOE=1} & Enable MXFP4 quantization for MoE models \\
\bottomrule
\end{tabular}
...
\begin{itemize}[noitemsep]
    \item \textbf{Ultra Ethernet Consortium (UEC)}: Industry consortium developing 1.6 Tbps lossless Ethernet for AI
    \item \textbf{OCP Falcon}: Meta's open networking specification
