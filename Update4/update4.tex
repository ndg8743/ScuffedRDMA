...
This update documents the discovery and resolution of a critical bug affecting FlashAttention 3 on NVIDIA Blackwell GPUs (RTX 5090). The fix enables our Cerberus node (2$\times$ RTX 5090, 64GB VRAM) to participate in multi-node vLLM inference with full FlashAttention 3 optimization.
...
\begin{enumerate}[noitemsep]
    \item \textbf{vLLM gpt-oss Blog Post} --- Confirmed gpt-oss requires FlashAttention with MXFP4
    \item \textbf{Kernel Fusion Article} --- Understood FlashInfer's role in MoE optimization
    \item \textbf{FlashInfer GitHub} --- Reviewed MXFP4 kernel implementation
    \item \textbf{vLLM Issue \#22279} --- Found the exact bug report and community fix
\end{enumerate}
...
This fix enables the full 136GB VRAM cluster (5 GPUs across 2 nodes) to run large models like DeepSeek-R1-70B and gpt-oss-120b with optimal performance.

\section{References}

\begin{itemize}[noitemsep]
    \item vLLM Issue \#22279: \url{https://github.com/vllm-project/vllm/issues/22279}
    \item vLLM gpt-oss Blog: \url{https://blog.vllm.ai/2025/08/05/gpt-oss.html}
    \item FlashInfer: \url{https://github.com/flashinfer-ai/flashinfer}
    \item FlashAttention Paper: \url{https://arxiv.org/abs/2307.08691}
\end{itemize}
